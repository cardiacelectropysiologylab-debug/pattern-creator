...
def main():
    ...
    # Remove edges to eliminate boundary artifacts
    # Use distance transform to keep only inner tissue
    dist_transform = ndimage.distance_transform_edt(tissue)
    # Keep only pixels that are at least 2 pixels away from boundary
    tissue_inner = dist_transform >= 2
    
    print(f"Tissue area: {tissue.sum():.0f} px² (inner: {tissue_inner.sum():.0f} px²)")
    
    # ======== STEP 3: DETECT HOLES WITHIN TISSUE ========
    print("\n[step3] Detecting holes...")
    
    # Use inner tissue (without edges) to detect holes and avoid boundary artifacts
    tissue_for_detection = tissue_inner
    
    # Strategy 1: Absolute darkness threshold - holes are very dark regardless of context
    # Empirical observation: holes have intensity < 1000
    ABSOLUTE_HOLE_THRESHOLD = 1000
    holes_absolute = (ref < ABSOLUTE_HOLE_THRESHOLD) & tissue_for_detection
    print(f"Absolute threshold: {ABSOLUTE_HOLE_THRESHOLD} (pixels below this are holes)")
    
    # Strategy 2: Relative darkness - much darker than tissue mean
    tissue_intensities = ref[tissue]
    tissue_mean = np.mean(tissue_intensities)
    tissue_std = np.std(tissue_intensities)
    tissue_percentile_90 = np.percentile(tissue_intensities, 90)
    
    print(f"Tissue stats: mean={tissue_mean:.0f}, std={tissue_std:.0f}, p90={tissue_percentile_90:.0f}")
    
    # Holes are pixels much darker than bright tissue
    hole_percentile = np.percentile(tissue_intensities, 60)
    hole_threshold = hole_percentile - 0.7 * tissue_std
    print(f"Relative threshold: {hole_threshold:.0f} (p60={hole_percentile:.0f} - 0.7*std)")
    
    holes_relative = (ref < hole_threshold) & tissue_for_detection
    
    # Strategy 3: Local contrast
    local_mean = cv2.blur(ref.astype(np.float32), (13, 13))
    holes_local = (ref < (local_mean - 0.5 * tissue_std)) & tissue_for_detection & (ref < 1263)
    
    # Combine all strategies (union)
    holes_combined = holes_absolute | holes_relative | holes_local
    ...
    for hole_id in range(1, n_holes_raw + 1):
        ...
        area = hole_mask.sum()
        if 5 <= area <= 6000:  # Lower minimum size + higher max
    ...
    axes[0, 2].imshow(holes_absolute, cmap='Reds', alpha=0.7)
    axes[0, 2].set_title("Holes (Absolute Intensity)", color='white', fontsize=11)
    axes[0, 2].axis('off')
    
    axes[1, 0].imshow(holes_relative, cmap='Oranges', alpha=0.7)
    axes[1, 0].set_title("Holes (Relative Darkness)", color='white', fontsize=11)
    axes[1, 0].axis('off')
