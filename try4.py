...
def add_compact(c, cx, cy, radius_pt, coverage, irregularity, offset_x_mm, offset_y_mm):
    ...
    Based on research: Creates a central fibrotic scar with:
    - Single compact region (unlike patchy which has multiple islands)
    - Irregular, organic boundary (using smooth Fourier harmonics)
    - Offset center (to model eccentric scarring)
    - Variable irregularity (0=perfect circle, 1=very irregular)
    """
    if coverage <= 0:
        return
    ...
    if irregularity <= 0.01:
        ...
        # Irregular blob using smooth sinusoid deformation
        n_points = 64
        points = []
...
def add_compact_preview(draw, cx, cy, radius_px, coverage, irregularity, offset_x_px, offset_y_px):
    """Preview for compact pattern with irregular scar shape
    
    Creates a movable, irregularly-shaped central scar with:
    - Smooth sinusoid-based boundary variations
    - Offset positioning for eccentric scarring
    - Variable irregularity control
    """
    if coverage <= 0:
        return
...
class PatternCreatorApp:
    ...
    def on_pattern_change(self, event=None):
        ...
        if pattern_type == "Interstitial":
            ...
            self._add_entry(self.params_frame, 2, "Offset X (mm):", self.var_offset_x, "mm",
                           callback=self.on_preview)
            self._add_entry(self.params_frame, 3, "Offset Y (mm):", self.var_offset_y, "mm",
                           callback=self.on_preview)
            
            # Info text for mouse dragging
    ...
    def on_canvas_drag(self, event):
        ...
        if self.compact_offset_mode and self.var_pattern_type.get() == "Compact":
            ...
            # Canvas is 800x600, image is 800x800 at zoom level
            diameter = MATTEK_SIZES[self.var_mattek_size.get()]
            circle_radius_mm = diameter / 2.0
            
            # Approximate conversion: canvas pixels to mm
            pixels_per_mm = 400 / circle_radius_mm  # Based on 800px image size
...
def generate_pattern(filename, pattern_type, coverage, circle_diameter_mm, white_border_fraction=0.15, seed=None, **kwargs):
    ...
    if pattern_type == "Interstitial":
        ...
        add_interstitial(c, cx, cy, pattern_radius_pt, inner_coverage,
                        rect_length_mm, rect_width_mm, spacing_along_mm, spacing_across_mm, angle_deg)
